...
/**
 * 
 * @returns Edit Shipping Address
 * @author Hrishikesh Kedar
 * @description A functional component which allows to edit shipping address
 * @param  navigation 
 */
...
const EditAddress = ({ navigation, route }) => {
    ...
    const [add, setAdd] = useState(item.addressLine)
    const [adderr, setAddErr] = useState();
    const [city, setCity] = useState(item.city)
    const [cityerr, setCityErr] = useState();
    const [pin, setPin] = useState(String(item.pincode))
    const [pinerr, setPinErr] = useState();
    const [state, setState] = useState(item.state)
    const [staterr, setStatErr] = useState();
    const [con, setCon] = useState(item.country)
